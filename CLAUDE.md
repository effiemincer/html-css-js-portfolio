...
- **Pesukim share button**: `.pesukim-share` is rendered per name-group by `buildNameHeader()` and handled via event delegation on `.pesukim-results` (alongside Show More and verse-expand clicks). It carries `data-name` and `data-total` attributes and uses two icon SVGs + two label `<span>`s — the `--success` variants are revealed by CSS when `data-copied="true"` is set. Removing the toolbar's old `.pesukim-copy-link` icon button or renaming `.pesukim-share` will break sharing.
