...
- **Pesukim toggles**: Three toggles in `.pesukim-controls` (language, sort, show-scores) are wired by class selector in `setupPesukim()` and persist to `localStorage` keys `pesukim-sort` and `pesukim-show-scores` (the language toggle is session-only). Renaming any of `.pesukim-lang-toggle`, `.pesukim-sort-toggle`, `.pesukim-score-toggle`, or their `__btn` children breaks the handler. The `.pesukim-copy-link` button in the same control bar is also wired by class selector and its disabled state is driven by `updateButtonState()`.
